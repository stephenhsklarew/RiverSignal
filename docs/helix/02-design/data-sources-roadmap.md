...
Combined design document covering: (1) data-source backlog with effort/impact ratings, (2) currently-shipped sources, and (3) state-by-state adapter inventory.

This document lives under `02-design/` because it is a planning artifact spanning many features (FEAT-005 ingestion + FEAT-007 fishing + FEAT-008 geology + FEAT-013 DeepTrail + FEAT-015 explore + FEAT-018 infra). Individual source ingestion features are tracked separately in `01-frame/features/`.

## Backlog (potential sources, not yet implemented)

> **Status legend**: 🟢 implemented | 🟡 partially implemented (different feature/cadence) | ⚪ not yet implemented

## High Value, Easy to Implement (REST/ArcGIS APIs, no key needed)

| Status | Source | Data | Value to App | Effort |
|--------|--------|------|-------------|--------|
| 🟢 | **NWS Forecast API** | 7-day weather by lat/lon | "Will it rain on my fishing trip?" on River Now hero card | Low — shipped as live source #27 |
| 🟢 | **USGS Real-Time Stream Gauges** | Live flow + temp (15-min intervals) | Replace monthly averages with *right now* readings on River Now | Low — shipped as live source #28 |
| 🟢 | **ODFW Fish Stocking Schedule** | Upcoming stocking dates, species, counts | "Trout stocking in 3 days at Lake Billy Chinook" alerts | Low — shipped as source #16 (953 records) |
...
| ⚪ | **USFS Trails** | 5,949 trail segments with surface, use type, mileage | Trail cards in Explore, trail lines on map | Medium — ArcGIS, line geometry. Note: USFS Recreation sites are shipped (#24) but trail-line geometry is not. |
...
| 🟡 | **SARP Barrier Inventory** (fix URL) | Fish passage barriers, waterfalls with height | Waterfall cards in Explore, barrier map overlay on species map | Medium — Fish Passage adapter shipped (#11, 476 barriers) but waterfall-specific data still pending |
| 🟢 | **OSMB Boating Access** | 1,815 boat ramps with amenities | Fill the boat ramp gap in Explore | Shipped as source #25 (160 sites) |
...
| 🟡 | **PRISM Real-Time Climate** | Daily precip, min/max temp grids | Actual current air temperature + recent rainfall on River Now | Medium — Monthly PRISM shipped (#12); daily resolution still pending |
| 🟡 | **Wildfire Perimeters (NIFC)** | Active + recent fire boundaries | Fire proximity warnings, recovery tracking on Steward | Medium — MTBS historical perimeters shipped (#8); current-year refresh pending |

## High Value, Higher Effort

| Status | Source                               | Data                                           | Value to App                                                               | Effort                                                                  |
| ------ | ------------------------------------ | ---------------------------------------------- | -------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| 🟢 | **Water Quality Portal — Chemistry** | Nutrient, turbidity, pH, conductivity readings | Water clarity metric (missing from hero card), nutrient trends for Steward | Shipped as source #3 (66K time series)                                  |
| 🟡 | **Westfly.com** hatch charts         | Expert hatch timing by river                   | Enrich curated hatch chart beyond our 50 entries                           | Curated hatch chart shipped (#26, 50 entries); Westfly enrichment still pending |
| 🟡 | **USDA SNOTEL** (expand)             | Real-time snowpack + soil moisture             | "Snowmelt runoff starting" context for spring flow predictions             | SNOTEL adapter shipped (#4, 650K time series). gold.snowpack_current view added 2026-05-09. Real-time + forecast interpretation still pending |
...
| Status | Source                         | Data                                      | Value                                                 |
| ------ | ------------------------------ | ----------------------------------------- | ----------------------------------------------------- |
| ⚪ | **Mindat.org API**             | Mineral locality data with photos         | Richer mineral site cards with specimen photos        |
| 🟢 | **GBIF** (Global Biodiversity) | Additional fossil + specimen records      | Fill fossil gaps beyond PBDB                          | Shipped as source #20 (965 fossils, 18 with images) |
