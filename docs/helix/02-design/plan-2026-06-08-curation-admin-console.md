...
#### Reach granularity — breaking out upstream tributaries
Watersheds often have upstream streams/brooks/creeks worth their own reach, especially where the state publishes reports/regulations for them (Shenandoah already runs 10 reaches for this reason). A reach is the app's pivot unit (own Go Score, Fish Present, hatch, flow bands), so each split is both an editorial promise and a data liability.
- **Split a tributary into its own reach when MOST hold:** (1) the state publishes **distinct regulations** for it (special-reg / C&R / artificial-only / seasonal closure — the strongest signal); (2) **distinct fishery** (different species than the mainstem); (3) **materially different conditions** (cold headwater vs. warm valley); (4) **anglers name and target it**. Fold it in when it's "more of the same water upstream."
- **Tier by data backing — never show a confident Go Score you can't ground.** *Full reach* = own gauge + flow bands → full-confidence TQS. *Reference/regulation reach* = surfaced for regs + species + hatch but **borrows conditions from the nearest gauged parent**, labeled and at lower confidence (reuse the `confidence` bucket; don't fabricate a flow score).
- **UX gap this exposes:** RiverPath is **watershed-scoped today** — no reach picker, and Fish Present / Catch Probability / Go Score are watershed-aggregated; `river_reaches` is flat (no `parent_reach_id`). So promoting a tributary is currently a data/curation change that does **not** surface as a browsable reach view. Realizing the value needs reach-navigation UX: a **two-level switcher** grouping tributaries under their parent water, **per-reach Fish Present / Go Score**, a **regulations surface** (badge + season/gear/limits), and **borrowed-condition labels** — plus an additive grouping field (`parent_reach_id` / `display_group`). Treat these as companion scope for any reach-granularity push.
- **Process:** ship coarse mainstem reaches at onboarding; promote a tributary **demand-driven** (reg published / distinct-fishery finding) via the Needs-Review queue, not enumerated up front. See runbook §2.4.1.
