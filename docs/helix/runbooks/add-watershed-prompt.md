...
**Watch for hand-maintained watershed ALLOWLISTS, not just display dicts.** Some files keep a
hardcoded `set`/`list` of valid slugs that mirrors `pipeline/config/watersheds.py` — e.g.
`app/routers/admin.py:VALID_WATERSHEDS` (rejects unknown slugs on the curated-photo editor). The
slug-grep catches these, but a missed one makes the endpoint **400/404 for the new watershed**
(`/admin/photos` "failed to load" for the new slug was exactly this). Prefer **deriving** such
allowlists from the config (`set(WATERSHEDS)`) rather than hand-copying, so they can't drift —
`admin.py` was converted to derive on 2026-06-03. Also: `sites.name` is set once by the Phase-1
seed migration, so a later display-name change in the config/frontend does NOT reach the existing
DB row — the `/riversignal` picker reads `site.name` (and strips " River"), so a stale suffix
(e.g. `Ipswich River (MA)` → "Ipswich (MA)") needs a `UPDATE sites SET name=...` migration, not
just a config edit. Keep the seeded `sites.name` suffix-free and consistent with the frontend label.
...
| `recreation_sites` rows | If RIDB has no coverage for the region (common for non-Western US), seed via alembic with `source_type='curated_<slug>_v0'` and the well-known state parks / NPS campgrounds / boat ramps / fishing access points / trailheads — at minimum 10 rows. **`rec_type` MUST be one of the `ExploreMapPage` FILTER keys** — `campground`, `trailhead`, `boat_ramp`, `fishing_access`, `fly_shop`, `guide_service`, `day_use`. A value like `state_park` is silently dropped from every chip (it only shows under "All"); map a state park to `campground`/`trailhead`/`fishing_access`/`day_use` instead. | `/path/explore` shows "No results", **or a chip is empty because rows used a `rec_type` that isn't a filter key** | Yes |
| `fly_shops_guides` / `mineral_shops` / `rockhounding_sites` rows | The §2.4 directory seeds are easy to skip and then `/path/explore`'s **Fly Shops** + **Guides** chips (and DeepTrail's shop/rockhound lists) are empty. Seed verified businesses (`needs_owner_verification`, approximate coords OK). **`fly_shops_guides.type` MUST be `fly_shop`, `guide_service`, or `both`** to match the ExploreMapPage filter — NOT `guide`/`shop`. `fly_shops_guides.id` is a serial — **omit `id` on INSERT** (don't pass `gen_random_uuid()`; the column is integer). | `/path/explore` Fly Shops + Guides chips empty; DeepTrail mineral-shop / rockhound lists empty | Yes |
...
| `gold.species_gallery` photos for hatch-chart insects | Run `pipeline refresh --mode heavy` (or trigger the prod `riversignal-refresh-heavy` job) after the watershed's iNaturalist ingest completes. The hatch endpoint joins curated hatch rows to species_gallery by genus + watershed. **WARNING:** until [bead RiverSignal-6a76a3ae](#) lands (unique indexes on heavy MVs), `refresh_heavy` takes a blocking exclusive lock and makes every `/path/now/*` and `/riversignal` page unreadable for ~30 minutes. Run this manually only after-hours, or refresh just the one MV via `psql -c "REFRESH MATERIALIZED VIEW gold.species_gallery"` and accept the same ~30 min lock on that single view. **Verify**: `SELECT COUNT(*) FROM gold.species_gallery WHERE watershed='<slug>'` returns >0 after refresh. If it returns 0 but observations exist, the iNat adapter may have dropped `photo_license` metadata for the watershed — see bead RiverSignal-713afbac. | "What Fish Are Eating Now" lists insects with no photos AND /riversignal Species tab is empty | Yes |
| Trigger `riversignal-pipeline-monthly` once for the new watershed | The monthly job (pbdb, idigbio, gbif, mrds, restoration, prism, recreation) is scheduled for the 1st of each month. If you onboard a watershed mid-month, the `/riversignal` Rocks tab + DeepTrail fossils/minerals will be empty until the next 1st. Trigger via `gcloud run jobs execute riversignal-pipeline-monthly --region us-west1 --wait` after the watershed's bbox is set in `pipeline/config/watersheds.py`. Note the job runs ALL adapters across ALL watersheds and takes 1–2h. | /riversignal Rocks tab returns `{fossils:0, minerals:0}`; DeepTrail tabs are empty | Yes |
| `/riversignal` initial map view must include the new watershed | `frontend/src/components/MapView.tsx` initialises the map with a `bounds` box that should cover every onboarded watershed's bbox. The original Pacific-NW-only `center: [-116, 43], zoom: 4.5` was widened to continental-US `[(-125, 24.5), (-66.5, 49.5)]` on 2026-05-16 — if a future watershed lands outside that box (e.g. Alaska or Hawaii), widen the bounds OR switch to a dynamic `fitBounds` over the sites array once it loads. | The watershed's bbox rectangle sits off-screen at first paint; user has to manually pan to find it | Yes |
| `/health` endpoint stays sub-100ms | Cloud Run startup + liveness probes hit `/health` every 30s. As the `observations` table grows past ~1M rows, never let `/health` do `SELECT count(*) FROM observations` or any other unindexed scan — that was dominating prod cold-start until 2026-05-16 (bead RiverSignal-7ea2ac57). The current implementation only runs `SELECT 1` against the DB pool. If you add a new diagnostic count to /health: put it on `/health/details` (or a new debug endpoint) and explicitly comment that it must not be wired into a probe. | Cloud Run logs show repeated `LIVENESS HTTP probe failed` / `STARTUP HTTP probe failed`; users see 30s timeouts during scale-up events | Yes |
