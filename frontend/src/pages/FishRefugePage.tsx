...
const SPECIES_TEMPS: Record<string, [number, number]> = {
  'oncorhynchus mykiss': [7, 16],       // Rainbow/Steelhead
  'salvelinus confluentus': [4, 12],     // Bull trout
  'oncorhynchus tshawytscha': [6, 14],   // Chinook
  'oncorhynchus kisutch': [7, 15],       // Coho
  'oncorhynchus nerka': [5, 14],         // Sockeye/Kokanee
  'salmo trutta': [7, 18],              // Brown trout
  'micropterus dolomieu': [15, 27],      // Smallmouth bass
}
...
export default function FishRefugePage() {
  ...
  const [ws, setWs] = useState(paramWs || 'deschutes')
  ...
  return (
    <div className="fish-page">
      {/* Watershed selector */}
      <div className="fish-ws-bar">
        {WATERSHEDS.map(w => (
          <button key={w} className={`fish-ws-btn${ws === w ? ' active' : ''}`} onClick={() => setWs(w)}>
