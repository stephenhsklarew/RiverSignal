...
-- The Utah adapter stores its JSON payload as text in interventions.description,
-- so we cast through jsonb. Filter to JSON-shaped descriptions defensively.
 SELECT DISTINCT
    i.site_id,
    s.watershed,
...
-- gold.stocking_schedule — UNION of ODFW + UDWR + WDFW stocking, with optional lat/lon
-- joined from the curated stocking_locations table (NULL when no entry exists).
...
-- ODFW (Oregon)
 SELECT t.site_id,
    ...
    sl.latitude, sl.longitude, sl.notes AS location_notes
   FROM time_series t
     JOIN sites s ON s.id = t.site_id
